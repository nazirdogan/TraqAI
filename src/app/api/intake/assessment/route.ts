...
export async function POST(request: Request) {
  ...
  const parsed = assessmentIntakeSchema.safeParse(
  ...
  if (!resendKey) {
    if (process.env.NODE_ENV !== 'production') {
      console.info('[intake/assessment] dev mode — skipping Resend. Intake:', intake);
  ...
  try {
    ...
    const leadSend = resend.emails.send({
      from: fromEmail,
      to: intake.email,
      subject: `Your AI readiness result: ${intake.band}`,
      react: AssessmentResultEmail({ intake }),
    });
...
export async function GET() {
  return NextResponse.json({ ok: true, endpoint: 'intake/assessment' });
}

  ...
  return (
    <section className="bg-bg-base px-5 pb-2 pt-2 sm:px-8 sm:pb-4">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col items-stretch divide-y divide-border-subtle rounded-[20px] border border-border-subtle bg-white p-3 shadow-card sm:flex-row sm:items-center sm:gap-2 sm:divide-y-0 sm:p-4">
          {ITEMS.map((item) => {
            const Icon = item.icon;
            return (
              <div key={item.title} className="flex flex-1 items-center gap-3 px-2 py-2.5 sm:py-1.5">
          ...
          <Link
            ...
            className="flex flex-1 items-center gap-3 rounded-[14px] px-2 py-2.5 transition-colors hover:bg-traq-tint sm:flex-none sm:px-4 sm:py-1.5"
          >
            <span className="flex h-9 w-9 flex-none items-center justify-center rounded-full bg-traq-tint text-traq-purple">
